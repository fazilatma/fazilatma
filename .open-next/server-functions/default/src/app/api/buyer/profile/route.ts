...
import { updateJsonBuyerProfile, type JsonKycDocument } from "@/lib/json-store";
...
async function saveOptionalKycDocuments(form: FormData) {
  ...
  if (nationalCard) savedDocuments.push(await saveKycFile(nationalCard, "national_card", "تصویر کارت ملی"));
  for (let index = 0; index < Math.min(8, birthCertificatePages.length); index += 1) {
    savedDocuments.push(await saveKycFile(birthCertificatePages[index], "birth_certificate", `صفحه ${index + 1} شناسنامه`));
  }
  if (bankCardImage) savedDocuments.push(await saveKycFile(bankCardImage, "bank_card", "تصویر کارت بانکی"));
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const buyer = await updateJsonBuyerProfile(Number(form.get("buyerId")), {
        ...
        phone: typeof form.get("phone") === "string" ? String(form.get("phone")) : undefined,
        bio: typeof form.get("bio") === "string" ? String(form.get("bio")) : undefined,
        city: typeof form.get("city") === "string" ? String(form.get("city")) : undefined,
        postalCode: typeof form.get("postalCode") === "string" ? String(form.get("postalCode")) : undefined,
        defaultAddress: typeof form.get("defaultAddress") === "string" ? String(form.get("defaultAddress")) : undefined,
        bankAccountHolder: typeof form.get("bankAccountHolder") === "string" ? String(form.get("bankAccountHolder")) : undefined,
        bankName: typeof form.get("bankName") === "string" ? String(form.get("bankName")) : undefined,
        bankAccountNumber: typeof form.get("bankAccountNumber") === "string" ? String(form.get("bankAccountNumber")) : undefined,
        bankCardNumber: typeof form.get("bankCardNumber") === "string" ? String(form.get("bankCardNumber")) : undefined,
        bankShebaNumber: typeof form.get("bankShebaNumber") === "string" ? String(form.get("bankShebaNumber")) : undefined,
        categories,
        avatarName: savedAvatarName || undefined,
        kycDocuments: savedDocuments,
      });
      return NextResponse.json({ success: true, buyer, message: "اطلاعات حساب خریدار ذخیره شد." });
    }
    ...
    const buyer = await updateJsonBuyerProfile(Number(body.buyerId), {
      ...
      bankAccountHolder: typeof body.bankAccountHolder === "string" ? body.bankAccountHolder : undefined,
      ...
      bankCardNumber: typeof body.bankCardNumber === "string" ? body.bankCardNumber : undefined,
      bankShebaNumber: typeof body.bankShebaNumber === "string" ? body.bankShebaNumber : undefined,
    ...
    const message = detail.includes("Profile image") || detail.includes("Only JPG")
      ...
      : detail.includes("Email already")
        ...
        : detail.includes("Phone already")
          ...
          : "ذخیره اطلاعات حساب خریدار ناموفق بود.";
