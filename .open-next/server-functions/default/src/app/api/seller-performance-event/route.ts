  ...
  try {
    ...
    if (!sellerId || !event) {
      return NextResponse.json(
        { success: false, message: "sellerId و event الزامی هستند." },
        { status: 400 },
      );
    }
    ...
    const increment = (key: keyof typeof m, by = 1) => ({
      [key]: (m[key] as number) + by,
    });
    ...
    switch (event) {
      ...
      case "seller_cancellation":
        updates = {
          ...
          stockMismatchCancellations90d:
            m.stockMismatchCancellations90d + (body.stockMismatch ? 1 : 0),
        };
        break;
      case "shipment_created": {
        const response = {
          ...
          trackedShipments90d:
            m.trackedShipments90d + (body.trackingProvided ? 1 : 0),
          validTrackedShipments90d:
            m.validTrackedShipments90d + (body.trackingValid ? 1 : 0),
        };
        updates = response;
        break;
      }
      case "buyer_review": {
        ...
        updates = {
          ...
          ratingAverage90d:
            (m.ratingAverage90d * m.reviewsCount90d + rating) / totalReviews,
        };
        break;
      }
      case "unresolved_case":
        updates = {
          ...increment("unresolvedCases90d"),
          ...increment("orderDefects90d"),
        };
        break;
      case "seller_fault_return":
        updates = {
          ...increment("sellerFaultReturns90d"),
          ...increment("orderDefects90d"),
        };
        break;
      case "dispute_against_seller":
        updates = {
          ...increment("disputesResolvedAgainstSeller90d"),
          ...increment("orderDefects90d"),
        };
        break;
      case "first_message_response": {
        ...
        updates = {
          ...
          firstMessagesAnsweredWithin24h:
            m.firstMessagesAnsweredWithin24h + (hours <= 24 ? 1 : 0),
          averageFirstResponseHours:
            (m.averageFirstResponseHours * m.firstMessagesReceived90d + hours) /
            totalMessages,
        };
        break;
      }
      case "verification_updated":
        updates = {
          ...
          profileCompletenessPercent: Math.max(
            0,
            Math.min(100, Number(body.profileCompletenessPercent) || 0),
          ),
        };
        break;
      default:
        return NextResponse.json(
          { success: false, message: "نوع رویداد پشتیبانی نمی‌شود." },
          { status: 400 },
        );
    }
    ...
    const detail =
      error instanceof Error
        ? error.message
        : "Unknown performance event error";
    return NextResponse.json(
      { success: false, message: "ثبت رویداد عملکرد ناموفق بود.", detail },
      { status: 500 },
    );
  }
}
