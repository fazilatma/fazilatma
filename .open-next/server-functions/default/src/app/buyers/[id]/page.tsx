    ...
    <div dir="rtl" className="min-h-screen bg-gray-50 pb-16">
      <div className="mx-auto max-w-5xl px-4 py-8">
        ...
        <section className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
          <div className="bg-gradient-to-l from-[#003b5c] to-[#005e94] p-8 text-white"><div className="flex items-center gap-5"><div className="flex h-24 w-24 items-center justify-center rounded-3xl bg-white/15 text-4xl font-bold">{entry.buyer.fullName.charAt(0)}</div><div><div className="flex flex-wrap items-center gap-3"><h1 className="text-3xl font-bold">{entry.buyer.fullName}</h1><SellerStars score={entry.rating * 20} size="sm" light /></div><p className="mt-2 text-blue-100">{entry.buyer.city || "موقعیت ثبت نشده"}</p></div></div></div>
        ...
        <section className="mt-6 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm"><h2 className="mb-5 text-xl font-bold">دیدگاه‌ها</h2>{reviews.length === 0 ? <p className="rounded-xl bg-gray-50 p-6 text-center text-sm text-gray-500">هنوز دیدگاهی ثبت نشده است.</p> : <div className="space-y-4">{reviews.map((review) => <article key={review.id} className="rounded-2xl border border-gray-100 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><p className="font-bold text-gray-900">{reviewerById.get(review.reviewerId) || "فروشنده"}</p><SellerStars score={review.overall * 20} size="sm" /></div><p className="mt-3 whitespace-pre-wrap text-sm leading-7 text-gray-600">{review.comment || "بدون توضیح"}</p><p className="mt-2 text-xs text-gray-400">سفارش {review.orderId}</p></article>)}</div>}</section>
      </div>
    </div>
  );
}
