        ...
        <div className="grid gap-6 md:grid-cols-3">
          <main className="space-y-6 md:col-span-2">
            ...
            <section className="rounded-3xl border border-gray-200 bg-white p-7 shadow-sm">
              ...
              {offers.length === 0 ? <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-10 text-center text-gray-500">هنوز فروشنده‌ای برای این درخواست پیشنهاد ثبت نکرده است.</div> : <div className="space-y-4">{offers.map((offer) => { const seller = sellerById.get(offer.sellerId); return <div key={offer.id} className="rounded-2xl border border-gray-200 p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row"><div><h3 className="font-bold text-[#003b5c]">{offer.sellerName}</h3><p className="mt-1 text-xs text-gray-500">زمان تحویل: {offer.deliveryDays} روز · وضعیت پیشنهاد: {offer.status}</p><p className="mt-3 text-sm text-gray-700">{offer.message || "توضیحی ثبت نشده است."}</p>{offer.productSpecs && <div className="mt-3 rounded-2xl border border-blue-100 bg-blue-50 p-3 text-xs text-blue-900"><p className="mb-2 font-bold">مشخصات کالای اعلام‌شده فروشنده</p><div className="grid gap-1 md:grid-cols-2"><span>برند/مدل: <b>{offer.productSpecs.brand} {offer.productSpecs.exactModel}</b></span><span>کانفیگ: <b>{offer.productSpecs.cpu} · {offer.productSpecs.ram} · {offer.productSpecs.storage}</b></span><span>سال ساخت: <b>{offer.productSpecs.manufactureYear}</b></span><span>وضعیت: <b>{offer.productSpecs.productCondition}</b></span><span>گارانتی: <b>{offer.productSpecs.warrantyStatus}</b></span><span>سلامت کلی: <b>{offer.productSpecs.partsHealth}</b></span></div><p className="mt-2 leading-6"><b>تعهد مرجوعی:</b> {offer.productSpecs.returnPolicy}</p></div>}{seller && <Link href={`/sellers/${seller.id}`} className="mt-3 inline-block text-sm font-bold text-[#00a8e8]">مشاهده پروفایل فروشنده</Link>}</div><div className="text-left"><p className="text-2xl font-bold text-[#0b9c56]">{Number(offer.amount).toLocaleString("fa-IR")} تومان</p><OfferAction offerStatus={offer.status} offerSellerId={offer.sellerId} requestBuyerId={request.buyerId} hasProductSpecs={Boolean(offer.productSpecs)} /></div></div></div>; })}</div>}
            </section>
          </main>
