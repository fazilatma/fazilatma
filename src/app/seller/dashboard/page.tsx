...
export default function SellerDashboardPage() {
  ...
  const [offerSpecs, setOfferSpecs] = useState<OfferSpecs>({
    ...defaultOfferSpecs,
  });
  ...
  const loadDashboard = async () => {
    if (!sellerId) {
      setError(
        "برای مشاهده داده‌های واقعی فروشنده، ابتدا با حساب فروشنده ثبت‌نام یا وارد شوید.",
      );
      setLoading(false);
      return;
    }
    ...
    try {
      const [dashboardResponse, scoreResponse] = await Promise.all([
        fetch(`/api/dashboard/seller?sellerId=${sellerId}`, {
          cache: "no-store",
        }),
      ...
      if (!dashboard.success)
        throw new Error(dashboard.message || "خطا در دریافت داشبورد فروشنده");
      setData(dashboard);
      ...
      setStoreAvatarPreview(
        dashboard.seller.avatarName
          ? `/api/avatar?userId=${dashboard.seller.id}&v=${encodeURIComponent(dashboard.seller.avatarName)}`
          : "",
      );
  ...
  const readyToShip = (data?.orders || []).filter(
  ...
  const archiveOrders = (data?.orders || []).filter(
    (order) =>
      order.sellerArchived ||
      ["completed", "cancelled", "returned"].includes(order.status),
  );
  const reviewedOrderIds = new Set(
    (data?.reviews || [])
      .filter((review) => review.reviewerId === data?.seller.id)
  ...
  const receivedReviews = (data?.reviews || []).filter(
    (review) => review.revieweeId === data?.seller.id,
  );
  ...
  const rejectCurrent = async () => {
    ...
    try {
      await action("/api/seller-request-action", {
  ...
  const submitOffer = async () => {
    ...
    try {
      const result = await action("/api/submit-offer", {
        sellerId: data.seller.id,
        requestId: currentRequest.id,
        amount,
      ...
      alert(err instanceof Error ? err.message : "ثبت پیشنهاد ناموفق بود");
    } finally {
  ...
  const shipOrder = async (orderId: string) => {
    ...
    if (!trackingCode.trim()) {
      alert("کد رهگیری ارسال را وارد کنید.");
      return;
    }
    try {
      const result = await action("/api/orders/ship", {
        sellerId: data.seller.id,
        orderId,
        trackingCode,
      });
      ...
      alert(err instanceof Error ? err.message : "تایید ارسال ناموفق بود");
    }
  };
  ...
  const archiveOrder = async (orderId: string) => {
    if (!data) return;
    try {
      const result = await action("/api/orders/archive", {
        userId: data.seller.id,
        orderId,
        role: "seller",
      });
  ...
  const saveSettings = async () => {
    ...
    try {
      ...
      alert(err instanceof Error ? err.message : "ذخیره تنظیمات ناموفق بود");
    }
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      const result = await action("/api/messages", {
        senderId: data.seller.id,
  ...
  if (loading)
    return (
      ...
      >
        در حال بارگذاری داشبورد فروشنده...
      </div>
    );
  if (error || !data)
    return (
    ...
    <div dir="rtl" className="min-h-screen bg-[#f8fcfb] pb-16">
        ...
        <section className="fixed inset-0 z-50 grid place-items-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="relative max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] border-4 border-[#0b9c56] bg-white p-6 shadow-2xl">
            ...
            <div className="relative z-10 flex items-start gap-3">
              <ProductThumb
                images={currentRequest.productImages}
            ...
            />
            <div className="relative z-10 mt-4 space-y-2 rounded-2xl bg-gray-50 p-4 text-sm">
              <p>
                دسته: <b className="text-blue-700">{currentRequest.category}</b>
              </p>
              <p>
                خریدار: <b>{currentRequest.buyerName}</b>
              </p>
              <p>
                تعداد: <b>{currentRequest.quantity}</b>
              </p>
              <p className="border-t pt-2">
                بودجه:{" "}
                <b className="text-lg text-green-600">
                  {money(currentRequest.budget)}
                </b>
              </p>
            </div>
            <div className="relative z-10 mt-4 rounded-2xl bg-blue-50 p-4">
              <p className="font-bold text-[#003b5c]">
                ثبت پیشنهاد و مشخصات کالا در صفحه جداگانه
              </p>
              <p className="mt-2 text-sm leading-7 text-blue-900">
                برای جلوگیری از بسته شدن فرم و ثبت دقیق مشخصات، قیمت و مشخصات
                کامل محصول پیشنهادی را در صفحه اختصاصی پر کنید.
              </p>
              <Link
                href={`/requests/${currentRequest.id}/offer`}
                className="mt-4 inline-block rounded-xl bg-[#003b5c] px-5 py-3 text-sm font-bold text-white hover:bg-[#002d46]"
              >
            ...
            <div className="relative z-10 mt-4 flex gap-3">
              ...
              <Link
                href={`/requests/${currentRequest.id}/offer`}
                className="flex-[2] rounded-xl bg-[#0b9c56] py-3 text-center font-bold text-white"
              >
                تکمیل فرم و ثبت پیشنهاد
              </Link>
            </div>
            <div
              className="absolute bottom-0 left-0 h-1.5 bg-[#0b9c56]"
      ...
      <div className="border-b bg-white px-4 py-3 shadow-sm">
        <div className="mx-auto flex max-w-6xl justify-between text-sm text-gray-500">
          <span>OptiBid / داشبورد فروشنده</span>
      ...
      <div className="mx-auto max-w-6xl px-4 py-8">
        <section className="mb-6 grid grid-cols-2 gap-4 md:grid-cols-4">
          <Metric
            value={remainingQueue.length}
            label="درخواست مرتبط"
            color="blue"
          />
          <Metric
            value={pendingPayment.length}
            label="منتظر پرداخت خریدار"
            color="amber"
          />
          <Metric
          ...
          />
          <Metric
            value={money(data.seller.walletBalance)}
            label="کیف پول فروشنده"
            color="green"
          />
        </section>
          ...
          <div className="flex flex-col justify-between gap-3 bg-gradient-to-l from-[#003b5c] to-[#005e94] p-5 text-white md:flex-row md:items-center">
            <div>
              <p className="text-sm font-bold text-blue-100">
                امتیاز اعتماد فروشنده
              </p>
              <h2 className="mt-1 text-2xl font-bold">
                {sellerScore?.label || "در حال ارزیابی"}
              </h2>
            </div>
            <div>
              <SellerStars
                score={sellerScore?.finalScore || 0}
                size="lg"
                light
              />
              <p className="mt-1 text-xs text-blue-100">
                اعتبار داده: {sellerScore?.confidence || 0}%
              </p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-px bg-gray-100 md:grid-cols-4">
            <Metric
              value={`${sellerScore?.rates.responseRate24h || 0}%`}
              label="پاسخ ۲۴ساعته"
              color="plain"
            />
            <Metric
              value={`${sellerScore?.rates.onTimeShippingRate || 0}%`}
              label="ارسال به‌موقع"
              color="plain"
            />
            <Metric
              value={`${sellerScore?.rates.validTrackingRate || 0}%`}
              label="رهگیری معتبر"
              color="plain"
            />
            <Metric
              value={`${sellerScore?.rates.defectRate || 0}%`}
              label="نرخ نقص"
              color="plain"
            />
          </div>
        ...
        <div className="mb-8 flex flex-wrap justify-center gap-2 rounded-[2rem] border border-gray-200 bg-white/70 p-4 shadow-sm">
          {tabs.map(([id, label, icon]) => (
            <button
              ...
              className={`rounded-full border px-4 py-2 text-sm font-bold ${activeTab === id ? "border-[#003b5c] bg-[#003b5c] text-white" : "border-gray-200 bg-white text-gray-600 hover:bg-blue-50"}`}
            >
              {icon} {label}
              {id === "requests" && remainingQueue.length
        ...
        {activeTab === "overview" && (
          <section className="grid gap-6 md:grid-cols-2">
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              <h2 className="mb-4 font-bold text-[#003b5c]">
                درخواست‌های مرتبط در صف
              </h2>
              {remainingQueue.length === 0 ? (
                <Empty text="درخواستی در حوزه‌های شما باقی نمانده است." />
              ) : (
                remainingQueue.map((req, index) => (
                  <div
                    key={req.id}
                    className="mb-3 flex justify-between rounded-xl bg-gray-50 p-3"
                  >
            ...
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              <h2 className="mb-4 font-bold text-[#003b5c]">
                سفارش‌های نیازمند ارسال
              </h2>
              {readyToShip.length === 0 ? (
                <Empty text="پس از پرداخت خریدار، سفارش‌ها اینجا می‌آیند." />
              ) : (
                readyToShip.map((order) => (
                  <div
                    key={order.id}
                    className="mb-3 rounded-xl bg-amber-50 p-3"
                  >
                    <div className="flex items-center gap-3">
                      <ProductThumb
                        images={
                      ...
                      <div className="flex min-w-0 gap-3">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="mt-2 text-sm text-gray-600">
                            {req.category} · {req.quantity} عدد · بودجه{" "}
                            {money(req.budget)}
                          </p>
                        </div>
                      </div>
        ...
        {activeTab === "orders" && (
          <section>
            <h1 className="mb-2 text-2xl font-bold text-[#003b5c]">سفارش‌ها</h1>
            <p className="mb-6 text-sm text-gray-500">
              ابتدا سفارش منتخب در انتظار پرداخت خریدار است؛ بلافاصله پس از
              پرداخت، همان سفارش آماده ارسال می‌شود.
            </p>
            <div className="mb-8">
              <h2 className="mb-3 text-lg font-bold text-amber-800">
                منتظر پرداخت خریدار ({pendingPayment.length})
              </h2>
              <div className="space-y-4">
                {pendingPayment.length === 0 ? (
                  <Empty text="هیچ سفارشی منتظر پرداخت خریدار نیست." />
                ) : (
                  pendingPayment.map((order) => (
                    <OrderCard key={order.id} order={order}>
                      <p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm text-amber-900">
                        پس از پرداخت خریدار، این سفارش به فهرست آماده ارسال
                        منتقل می‌شود.
                      </p>
                    </OrderCard>
                  ))
            ...
            <div>
              <h2 className="mb-3 text-lg font-bold text-[#0b9c56]">
                پرداخت‌شده و آماده ارسال ({readyToShip.length})
              </h2>
              <div className="space-y-4">
                {readyToShip.length === 0 ? (
                  <Empty text="سفارش پرداخت‌شده و آماده ارسال ندارید." />
                ) : (
                  readyToShip.map((order) => (
                    <OrderCard key={order.id} order={order}>
                      <div className="mt-4 flex items-center justify-between rounded-xl bg-green-50 p-3 text-sm text-green-900">
                        <span>
                          خریدار پرداخت را انجام داده؛ کالا را به آدرس ثبت‌شده
                          ارسال کنید.
                        </span>
                        <button
                          onClick={() => setActiveTab("shipping")}
                          className="rounded-lg bg-[#0b9c56] px-4 py-2 font-bold text-white"
                        >
                          ثبت ارسال
                        </button>
                      </div>
        ...
        {activeTab === "shipping" && (
          <section>
            ...
            <p className="mb-6 text-sm text-gray-500">
              هر سفارش پرداخت‌شده را جداگانه ارسال و کد رهگیری را ثبت کنید؛ این
              کار اعلان خریدار را ایجاد می‌کند.
            </p>
            <div className="space-y-4">
              {readyToShip.length === 0 ? (
                <Empty text="سفارش پرداخت‌شده و آماده ارسال ندارید." />
                    ...
                    <div className="mt-4 rounded-xl bg-blue-50 p-4">
                      ...
                      <div className="mt-4 flex gap-3">
                        <input
                          ...
                          className="flex-1 rounded-xl border bg-white p-3 text-sm outline-none focus:border-[#00a8e8]"
                        />
                        <button
                          onClick={() => shipOrder(order.id)}
                          className="rounded-xl bg-[#0b9c56] px-5 py-3 font-bold text-white"
                        >
                          ✓ تایید ارسال کالا
                        </button>
                      </div>
        ...
        {activeTab === "wallet" && (
          <section className="space-y-6">
            <div className="grid gap-6 md:grid-cols-2">
              <div className="rounded-3xl bg-gradient-to-br from-[#003b5c] to-[#005e94] p-6 text-white shadow-lg">
                ...
                <p className="mt-3 text-3xl font-bold">
                  {money(data.seller.walletBalance)}
                </p>
                <p className="mt-5 text-sm leading-7 text-blue-100">
                  فقط پس از تایید دریافت کالا توسط خریدار، سهم خالص فروشنده پس
                  از کسر کمیسیون به این کیف پول افزوده می‌شود.
                </p>
              </div>
              <div className="rounded-3xl border border-blue-100 bg-white p-6 shadow-sm">
                <h2 className="font-bold text-[#003b5c]">
                  برداشت موجودی به حساب فروشنده
                </h2>
                <div className="mt-4 rounded-xl bg-gray-50 p-4 text-sm leading-7 text-gray-600">
                  <p>
                    <b>صاحب حساب:</b>{" "}
                    {data.seller.bankAccountHolder || "ثبت نشده"}
                  </p>
                  <p>
                    <b>بانک:</b> {data.seller.bankName || "ثبت نشده"}
                  </p>
                  <p dir="ltr" className="text-right">
                    <b>کارت:</b> {maskCard(data.seller.bankCardNumber)}
                  </p>
                  <p dir="ltr" className="text-right">
                    <b>شبا:</b> {maskSheba(data.seller.bankShebaNumber)}
                  </p>
                  <p
                    className={`mt-2 text-xs font-bold ${data.seller.bankDetailsVerified ? "text-green-600" : "text-amber-600"}`}
                  >
                    {data.seller.bankDetailsVerified
                      ? "✓ اطلاعات بانکی تایید شده"
                      : "درخواست برداشت توسط ادمین بررسی می‌شود"}
                  </p>
                </div>
                <input
            ...
            <div className="grid gap-6 lg:grid-cols-2">
              <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                <h2 className="mb-4 font-bold">گردش کیف پول فروشنده</h2>
                {data.transactions.length === 0 ? (
                  <Empty text="هنوز تراکنش واریز یا تسویه ندارید." />
                ) : (
              ...
              <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                {data.withdrawals.length === 0 ? (
                  <Empty text="هنوز درخواست برداشتی ندارید." />
                ) : (
        ...
        {activeTab === "survey" && (
          <section>
            <h1 className="mb-2 text-2xl font-bold text-[#003b5c]">
              ارزیابی خریداران معاملات تکمیل‌شده
            </h1>
            <p className="mb-6 text-sm text-gray-500">
              این امتیازها مبنای انتخاب خریداران برتر OptiBid هستند.
            </p>
        ...
        {activeTab === "reviews" && (
          <section>
            ...
            <p className="mb-6 text-sm text-gray-500">
              توضیحاتی که خریداران در فرم نظرسنجی درباره شما نوشته‌اند، اینجا
              نمایش داده می‌شود.
            </p>
            <div className="space-y-4">
          ...
          <section>
            ...
            <div className="space-y-4">
              {archiveOrders.length === 0 ? (
                <Empty text="پس از تکمیل یا لغو سفارش، امکان بایگانی آن فعال می‌شود." />
              ) : (
        ...
        {activeTab === "messages" && (
          <section className="grid min-h-[480px] overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm md:grid-cols-3">
            <div className="border-l bg-gray-50 p-4">
              <h2 className="mb-4 font-bold">گفتگو با خریداران</h2>
              {counterpartUsers.length === 0 ? (
                <p className="text-sm text-gray-500">
                  پس از دریافت درخواست یا سفارش، گفتگوها در این بخش فعال
                  می‌شوند.
                </p>
              ) : (
                counterpartUsers.map((user) => (
                  <button
                    ...
                    className={`mb-2 w-full rounded-xl p-3 text-right text-sm font-bold ${activeChatUserId === user.id ? "bg-blue-100 text-[#003b5c]" : "bg-white text-gray-600"}`}
                  >
                    {user.name}
                  </button>
                ))
            ...
            <div className="flex flex-col p-5 md:col-span-2">
              <div className="flex-1 space-y-3 overflow-y-auto">
                {currentMessages.length === 0 ? (
                  ...
                  currentMessages.map((message) => (
                    <div
                      key={message.id}
                      className={`max-w-[75%] rounded-2xl p-3 text-sm ${message.senderId === data.seller.id ? "mr-auto bg-[#003b5c] text-white" : "bg-gray-100 text-gray-800"}`}
                    >
                      {message.content}
              ...
              <div className="mt-4 flex gap-2">
                <input
                  ...
                  placeholder="پیام به خریدار..."
                  className="flex-1 rounded-xl border p-3 text-sm outline-none focus:border-[#00a8e8]"
                />
                <button
                  onClick={sendMessage}
        ...
        {activeTab === "notifications" && (
          <section>
            <h1 className="mb-6 text-2xl font-bold text-[#003b5c]">اعلان‌ها</h1>
            <div className="overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm">
              {data.notifications.length === 0 ? (
                <Empty text="هنوز اعلانی ندارید." />
              ) : (
                data.notifications.map((n) => (
                  <div key={n.id} className="border-b p-5">
                    <div className="flex justify-between gap-4">
                      <div>
        ...
        {activeTab === "settings" && (
          <section className="mx-auto max-w-4xl rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
              ...
              <div className="flex-1 text-center sm:text-right">
                ...
                <label className="mt-3 inline-flex cursor-pointer rounded-xl bg-blue-50 px-4 py-2 text-sm font-bold text-[#00a8e8] hover:bg-blue-100">
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={(event) =>
                      handleStoreAvatarChange(event.target.files?.[0])
                    }
                  />
                </label>
              </div>
            ...
            />
            <h2 className="mt-6 border-b pb-3 text-sm font-bold">
              حوزه‌های فعالیت — فقط این درخواست‌ها به رادار شما می‌آیند
            </h2>
            <div className="mt-4 grid gap-3 sm:grid-cols-2 md:grid-cols-3">
              {Object.values(categoryMap).map((category) => (
                <label
                  key={category}
                  className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 text-sm font-bold ${settings.categories.includes(category) ? "border-[#00a8e8] bg-blue-50" : "bg-gray-50"}`}
                >
                  <input
                    type="checkbox"
                    ...
                    onChange={() =>
                      setSettings({
                        ...
                        categories: settings.categories.includes(category)
                          ? settings.categories.filter((x) => x !== category)
...
function Metric({
  ...
  value: string | number;
  label: string;
  color: "blue" | "amber" | "orange" | "green" | "plain";
}) {
  const c = {
    blue: "border-b-4 border-blue-500",
    amber: "border-b-4 border-amber-500",
    orange: "border-b-4 border-orange-500",
    green: "border-b-4 border-[#00a8e8]",
    plain: "",
  }[color];
  return (
    <div className={`rounded-2xl bg-white p-5 text-center shadow-sm ${c}`}>
      <p className="text-2xl font-bold text-[#003b5c]">{value}</p>
    ...
    <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
      <div className="flex flex-col justify-between gap-3 sm:flex-row">
        ...
        <div className="text-left">
          ...
          <p className="mt-1 text-xs text-gray-500">
            خالص فروشنده پس از کمیسیون: {money(order.sellerAmount)}
          </p>
        </div>
      </div>
      {children}
    ...
    <div className="rounded-2xl border border-blue-100 bg-white p-4 text-right">
      <div className="mb-4 border-b border-gray-100 pb-3">
        <h3 className="font-bold text-[#003b5c]">
          مشخصات واقعی کالای پیشنهادی فروشنده
        </h3>
        <p className="mt-1 text-xs leading-6 text-gray-500">
          این مشخصات قبل از پرداخت به خریدار نمایش داده می‌شود و خریدار باید آن
          را تایید کند. تمام فیلدهای اصلی را دقیق وارد کنید.
        </p>
      </div>
