...
export default async function SellerProfilePage({ params }: SellerPageProps) {
  ...
  const reviews = data.reviews
    .filter(
      (review) =>
        review.revieweeId === seller.id && review.reviewerRole === "buyer",
    )
    .sort(
      (a, b) =>
  ...
  const completedOrders = data.orders
    .filter(
      (order) => order.sellerId === seller.id && order.status === "completed",
    )
    .sort(
      (a, b) =>
  ...
  const acceptedOffers = sellerOffers.filter(
  ...
  const netSellerRevenue = completedOrders.reduce(
    (sum, order) => sum + Number(order.sellerAmount || 0),
    0,
  );
  const averageSaleAmount = Math.round(
    totalSalesAmount / Math.max(1, completedOrders.length),
  );
  const successRate =
    completedOrders.length + failedOrders.length === 0
      ? 0
      : Math.round(
  ...
  const avatarUrl = seller.avatarName
    ? `/api/avatar?userId=${seller.id}&v=${encodeURIComponent(seller.avatarName)}`
    : "";
  const categoryStats = [
    ...completedOrders
      .reduce((map, order) => {
        const current = map.get(order.category) || {
        ...
        return map;
      }, new Map<string, { category: string; count: number; amount: number }>())
      .values(),
  ...
  const metricRows = [
    {
      ...
      value: score.metrics.reliability,
      weight: 30,
      icon: "🛡️",
    },
    {
      title: "ارسال و تحویل",
      value: score.metrics.fulfilment,
      weight: 25,
      icon: "🚚",
    },
    {
      title: "رضایت و پاسخ‌گویی",
      value: score.metrics.buyerExperience,
      weight: 25,
      icon: "💬",
    },
    {
      title: "اعتماد و انطباق",
      value: score.metrics.trust,
      weight: 12,
      icon: "✅",
    },
    {
      title: "سابقه و فعالیت",
      value: score.metrics.experience,
      weight: 8,
      icon: "📈",
    },
  ];
    ...
    <div dir="rtl" className="min-h-screen bg-gray-50 pb-16">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div className="mb-6 text-sm text-gray-500">
          ...
          <Link href="/sellers" className="hover:text-[#00a8e8]">
            فروشندگان
          </Link>
          <span className="mx-2">/</span>
        ...
        <section className="overflow-hidden rounded-[2rem] border border-gray-200 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-center">
                ...
                <div className="flex-1">
                  <div className="mb-2 flex flex-wrap items-center gap-3">
                    ...
                    <span
                      className={`rounded-full border px-3 py-1 text-xs font-bold ${ratingLevelClass(score.level)}`}
                    >
                      {score.label}
                    </span>
                    {seller.sellerMetrics?.identityVerified && (
                      <span className="rounded-full bg-blue-400/20 px-3 py-1 text-xs font-bold">
                        تایید هویت
                      </span>
                    )}
                  </div>
                  <p className="text-blue-100">
                    {seller.categories?.join("، ") || "حوزه فعالیت ثبت نشده"}
                  </p>
                  <p className="mt-3 max-w-2xl text-sm leading-7 text-blue-100">
                    {seller.bio ||
                      "این فروشنده هنوز توضیحات پروفایل خود را تکمیل نکرده است."}
                  </p>
              ...
              <div className="flex flex-col gap-3">
                <Link
                  ...
                  className="rounded-xl bg-white px-5 py-3 text-center font-bold text-[#003b5c] transition hover:bg-blue-50"
                >
                  ثبت درخواست خرید
                </Link>
          ...
          <div className="grid gap-4 p-6 md:grid-cols-2 xl:grid-cols-5">
            <StatCard
              label="فروش موفق"
              value={completedOrders.length.toLocaleString("fa-IR")}
              color="green"
            />
            <StatCard
              label="فروش ناخالص"
            ...
            <StatCard
              ...
              value={money(netSellerRevenue)}
              color="purple"
            />
            <StatCard
              label="میانگین فروش"
              value={money(averageSaleAmount)}
              color="amber"
            />
            <StatCard
              label="دیدگاه خریداران"
              value={reviews.length.toLocaleString("fa-IR")}
              color="slate"
            />
          </div>
          ...
          <div className="grid gap-6 border-t p-8 lg:grid-cols-3">
            <div className="rounded-2xl border border-blue-100 bg-blue-50 p-6 text-center">
              ...
              <div className="mt-3">
                <SellerStars score={score.finalScore} size="lg" />
              </div>
              <p className="mt-2 text-xs text-gray-500">
                امتیاز محاسباتی: {score.finalScore.toLocaleString("fa-IR")} از
                ۱۰۰
              </p>
              <span
                className={`mt-4 inline-flex rounded-full border px-3 py-1 text-xs font-bold ${ratingLevelClass(score.level)}`}
              >
                {score.label}
              </span>
            </div>
            <div className="lg:col-span-2">
              <h2 className="mb-4 text-lg font-bold text-gray-900">
                گزارش عملکرد ۹۰ روز اخیر
              </h2>
              <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                <MiniMetric
                  label="امتیاز تعدیل‌شده"
                  value={score.rates.bayesianRating.toLocaleString("fa-IR")}
                />
                <MiniMetric
                  label="ارسال به‌موقع"
                  value={`${score.rates.onTimeShippingRate.toLocaleString("fa-IR")}%`}
                />
                <MiniMetric
                  label="رهگیری معتبر"
                  value={`${score.rates.validTrackingRate.toLocaleString("fa-IR")}%`}
                />
                <MiniMetric
                  label="پاسخ زیر ۲۴ ساعت"
                  value={`${score.rates.responseRate24h.toLocaleString("fa-IR")}%`}
                />
              </div>
            </div>
          </div>
            ...
            <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-xl font-bold text-gray-900">
                  فروش‌های موفق فروشنده
                </h2>
                <p className="mt-1 text-sm text-gray-500">
                  سفارش‌هایی که خریدار دریافت کالا را تایید کرده و وجه آزاد شده
                  است.
                </p>
              </div>
              <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-bold text-green-700">
                {completedOrders.length.toLocaleString("fa-IR")} فروش موفق
              </span>
            </div>
            {completedOrders.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-10 text-center text-sm text-gray-500">
                هنوز فروش موفقی برای این فروشنده ثبت نشده است.
              </div>
            ) : (
                    ...
                    <div className="flex flex-col justify-between gap-3 md:flex-row md:items-start">
                      <div>
                        ...
                        <p className="mt-1 text-xs text-gray-500">
                          خریدار: {order.buyerName} · تاریخ تکمیل:{" "}
                          {dateLabel(order.deliveredAt || order.createdAt)}
                        </p>
                      </div>
                      <div className="text-left">
                        <p className="text-lg font-bold text-[#0b9c56]">
                          {money(order.totalAmount)}
                        </p>
                        <p className="mt-1 text-xs text-gray-500">
                          خالص فروشنده: {money(order.sellerAmount)}
                        </p>
                    ...
                    >
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <span className="text-gray-500">
                          {item.count.toLocaleString("fa-IR")} فروش
                        </span>
                      </div>
                      <div className="mt-2 h-2 overflow-hidden rounded-full bg-gray-200">
                        <div
                          className="h-full rounded-full bg-gradient-to-l from-[#003b5c] to-[#00a8e8]"
                          style={{
                            width: `${Math.min(100, (item.amount / Math.max(1, totalSalesAmount)) * 100)}%`,
                          }}
                        />
                      </div>
                      <p className="mt-2 text-xs font-bold text-[#003b5c]">
                        {money(item.amount)}
                      </p>
                    </div>
                  ))}
            ...
            <div className="rounded-3xl border border-gray-200 bg-white p-6 shadow-sm">
              ...
              <div className="grid gap-3 text-sm">
                <QuickStat
                  ...
                  value={sellerOffers.length.toLocaleString("fa-IR")}
                />
                <QuickStat
                  label="پیشنهادهای پذیرفته‌شده"
                  value={acceptedOffers.length.toLocaleString("fa-IR")}
                />
                <QuickStat
                ...
                />
                <QuickStat
                  label="فروش ناموفق/لغوشده"
                  value={failedOrders.length.toLocaleString("fa-IR")}
                />
              </div>
            </div>
          </div>
        ...
        <section className="mt-6 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
          <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-sm text-gray-500">
                دیدگاه‌هایی که پس از معامله درباره این فروشنده ثبت شده‌اند.
              </p>
            </div>
          ...
          {reviews.length === 0 ? (
            ...
            <div className="space-y-4">
              {reviews.map((review) => {
                const reviewer = data.users.find(
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        <div className="flex h-11 w-11 items-center justify-center overflow-hidden rounded-2xl bg-[#003b5c] text-sm font-bold text-white">
                          {reviewer?.avatarName ? (
                            <img
                              src={`/api/avatar?userId=${reviewer.id}&v=${encodeURIComponent(reviewer.avatarName)}`}
                              alt={reviewer.fullName}
                              className="h-full w-full object-cover"
                            />
                          ) : (
                            (reviewerById.get(review.reviewerId) || "خ").charAt(
                              0,
                            )
                          )}
                        </div>
                        <div>
                          <p className="font-bold text-gray-900">
                            {reviewerById.get(review.reviewerId) || "خریدار"}
                          </p>
        ...
        <section className="mt-6 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-xl font-bold text-gray-900">
              جزئیات امتیازدهی شفاف
            </h2>
            <span className="text-sm text-gray-500">
              اعتبار داده: {score.confidence}%
            </span>
          </div>
              ...
              <div key={metric.title}>
                <div className="mb-2 flex justify-between text-sm">
                  ...
                  <span className="font-bold text-[#003b5c]">
                    {metric.value.toLocaleString("fa-IR")} / ۱۰۰
                  </span>
                </div>
          ...
          {score.minimumDataMessage && (
            <p className="mt-6 rounded-xl bg-slate-50 p-4 text-sm leading-7 text-slate-600">
              {score.minimumDataMessage}
            </p>
          )}
          {score.reasons.length > 0 && (
            <p className="mt-4 rounded-xl bg-red-50 p-4 text-sm leading-7 text-red-700">
              هشدارهای عملکرد: {score.reasons.join("، ")}
            </p>
          )}
        </section>
        ...
        <section className="mt-6 rounded-3xl border border-gray-200 bg-white p-8 shadow-sm">
          <h2 className="mb-4 text-xl font-bold text-gray-900">
            شاخص‌های کنترلی OptiBid
          </h2>
          <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
            {SELLER_RATING_CRITERIA.map((criterion) => (
              <div key={criterion.key} className="rounded-xl bg-gray-50 p-4">
...
function MiniMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl bg-gray-50 p-4 text-center">
      <p className="text-lg font-bold text-[#003b5c]">{value}</p>
...
function QuickStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between rounded-xl bg-gray-50 px-3 py-2">
      ...
      <b className="text-[#003b5c]">{value}</b>
    </div>
  );
}
